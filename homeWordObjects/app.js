...
const studentsWithAvg = students.map((student) => {
    return {
        ...student,
        average:
            student.grades.reduce((prev, curr) => prev + curr / student.grades.length)
    }
})

const studentiZaStipendiju = studentsWithAvg.filter((student) => student.average > 8)
...
const person = {
    ...
    getNickName: function() {
        return this.name.slice(0,2) + this.surname.slice(0,2);
    }
}

const nick = person.getNickName()
